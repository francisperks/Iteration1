...
/**
 * Acts as an overall container for any Phaser.Sound.BaseSounds made. Is attached to
 * a scene so needs recreating for each new scene in the game.
 */
class AudioManager {
    /**
     * @constructor
     * @param {Phaser.Scene} scene - The Scene to which this Game Object belongs. A Game Object can only belong to one Scene at a time.
     * @param {number?} volume - (Optional) The volume that all sounds within the AudioManager should be set to. Ranges from 0 to 1. Default 1.
     * @param {boolean?} muted - (Optional) Whether all sounds in the AudioManager are muted or not. Default false.
     */
    ...
    /**
     * Adds a new BaseSound to the scene, which the AudioManager keeps track of.
     * @param {string} key - The key of the preloaded sound that this BaseSound will use. This same key will be used to access the BaseSound.
     * @param {Phaser.Types.Sound.SoundConfig?} config - (Optional) A config object containing default sound settings.
     */
    addAudio(key, config = {}) {
    ...
    /**
     * Changes the volune for all sounds in the AudioManager to the number provided.
     * @param {number} volume - The value to set the AudioManager's volume to. From 0 to 1. 
     */
    setVolume(volume) {
        this.volume = volume;
    ...
    /**
     * Set all sounds in the AudioManager to being muted.
     */
    mute() {
    ...
    /**
     * Set all sounds in the AudioManager to being unmuted.
     */
    unmute() {
    ...
    /**
     * Toggle between all sounds in the AudioManager being muted and unmuted.
     */
    toggleMute() {
        if(this.muted) {
            this.unmute();
    ...
    /**
     * Stop playing all sounds in the AudioManager.
     */
    stopAllAudio() {
        for(let sound of Object.values(this.audioList)) {
            sound.stop();
